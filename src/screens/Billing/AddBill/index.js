...
const FeedBill = () => {
    ...
    const generateType4Options = () => {
        const menuItemsHTML = [];
        BillData.StockTypes.stockType4.forEach(((data, idx) => {
    ...
    const generateType3Options = () => {
        const menuItemsHTML = [];
        BillData.StockTypes.statusType.forEach(((data, idx) => {
            menuItemsHTML.push(<option value={data.key}>{data.value}</option>);
        }));
        return menuItemsHTML;
    ...
    const generateOptions = () => {
        const menuItemsHTML = [];
        if(stockType === 'looms'){
            BillData.StockTypes.loomsType.forEach(((data, idx) => {
                menuItemsHTML.push(<option value={data.key}>{data.value}</option>);
            }));
            return menuItemsHTML;
        }
        else if (stockType === 'dobby'){
            BillData.StockTypes.dobbyType.forEach(((data, idx) => {
                menuItemsHTML.push(<option value={data.key}>{data.value}</option>);
            }));
            return menuItemsHTML;
        }
